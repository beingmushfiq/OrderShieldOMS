...
const mockInvoices: Invoice[] = [
  {
    ...
    dueDate: 'Nov 24, 2026',
    items: [
      { description: 'Tin Goyenda Collector Set', quantity: 1, unitPrice: 2499, total: 2499 },
    ]
  },
  {
    id: 'INV-2026-002',
    ...
    total: 1840.00,
    status: 'pending',
    issuedDate: 'Oct 23, 2026',
    dueDate: 'Nov 23, 2026',
    items: [
      { description: 'Sapiens: A Brief History of Humankind', quantity: 1, unitPrice: 1600, total: 1600 },
    ]
  },
  {
    id: 'INV-2026-003',
    ...
    dueDate: 'Oct 20, 2026',
    items: [
      { description: 'Shawshank Redemption (Hardcover)', quantity: 1, unitPrice: 447, total: 447 },
    ]
  },
];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-wrap gap-2">
          {(['all', 'paid', 'pending', 'overdue'] as const).map((f) => (
            ...
            >
              {f} ({f === 'all' ? mockInvoices.length : mockInvoices.filter(i => i.status === f).length})
            </button>
          ))}
        </div>
          ...
          <>
            ...
            >
              <div className="p-6 flex items-center justify-between border-b border-outline-variant/10">
                ...
                <button onClick={() => setSelectedInvoice(null)} className="p-2 hover:bg-surface-container-high rounded-full transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-10">
                ...
                <div className="bg-surface-container-high rounded-3xl p-8 space-y-4 border border-outline-variant/10 shadow-xl">
                  ...
                  <div className="flex justify-between text-xs font-bold uppercase tracking-widest">
                    <span className="text-on-surface-variant">Tax (15%)</span>
                    <span className="text-on-surface">৳ {(selectedInvoice.tax || 0).toLocaleString()}</span>
                  </div>
              ...
              <div className="p-8 bg-surface-container mt-auto border-t border-outline-variant/10 grid grid-cols-2 gap-4">
                <button className="py-4 bg-surface-container-high rounded-2xl text-[10px] font-black uppercase tracking-widest text-primary hover:bg-surface-container-highest transition-all flex items-center justify-center gap-3 border border-primary/10">
                  <Printer size={16} />
                  Print
                </button>
                <button className="py-4 bg-gradient-to-br from-primary to-primary-container text-on-primary rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">
                  <Send size={16} />
                  Send
                </button>
              </div>
